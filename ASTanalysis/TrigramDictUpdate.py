...
def funcOfControl(controlfilelist, destPath, pathToEmptyDict, dictFile):
    ...
    for eachPath in controlfilelist:
        ...
        for i in range(len(treeNodes)-2):
            index = dictFile[treeNodes[i]] + '$$' + dictFile[treeNodes[i+1]] + '$$' + dictFile[treeNodes[i+2]]
            emptyDict[index] += 1.0
...
def main():
    ...
    pathToEmptyDict = "/Users/sbukhari/Sandbox/CProgram/Processed/EmptyDictionaries/dictOfTrigram.pickle"
    ...
    with open("/Users/sbukhari/Sandbox/CProgram/Processed/DictOfNodes.pickle", "rb") as inF:
    ...
    cmdparser.add_argument('parent', help='Add path for the folder that contains pickle files for each function in two folders Control & Autopilot')
